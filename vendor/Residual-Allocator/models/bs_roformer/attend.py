...
class Attend(nn.Module):
    def __init__(
        ...
        self.cuda_math_config = FlashAttentionConfig(False, True, False)
    ...
    def flash_attn(self, q, k, v):
        ...
        try:
            ...
            fallback_config = self.cuda_math_config if is_cuda else self.cpu_config
            with torch.backends.cuda.sdp_kernel(**fallback_config._asdict()):
                out = F.scaled_dot_product_attention(
                    q, k, v,
                    dropout_p = self.dropout if self.training else 0.
                )
